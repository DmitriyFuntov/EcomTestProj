...
1. Собрать jar файл (testproj-1.0-SNAPSHOT.jar(должен оказаться в папке target))
2. В папке testproj создать .env файл с переменными `minio_user` и `minio_password`
3. В этой же папке выполнить сначала `docker-compose build`, затем `docker-compose up`
4. Дождаться выполнения скрипта, перейти по ссылке http://localhost:9001 и авторизоваться (значения `minio_user` и `minio_password`)
...
5. Далее при замене файла, просто перезапускать контейнер spark-app
